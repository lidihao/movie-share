    ...
    <table class="table table-bordered" id='hl-tree-table'>
      ...
      <tr v-for="(item,index) in initItems" :key="item.id" v-show="show(item)" :class="{'child-tr':item.parent}">
        <td v-for="(column,snum) in columns" :key="column.key" :style=tdStyle(column)>
          <label v-if="column.type === 'selection'">
            <input type="checkbox" :value="item.id" v-model="item.isChecked" @click="handleCheckClick(item,$event,index)">
          </label>
          <div v-if="column.type === 'action'">
            <Button  style="margin: 5px" :type="action.type" size="small" @click="RowClick(item,$event,index,action.text)" v-for='action in (column.actions)' :key="action.text">{{action.text}}</Button>
          </div>
          <label @click="toggle(index,item)" v-if="!column.type" :style="caculatePadding(item)">
...
<script>
  export default {
    ...
    methods: {
      ...
      caculatePadding(item){
        let style={}
        style['padding-left']=7*item.level+'px';
        return style
      },
      // 排序事件
      ...
      handleCheckClick(data,event,index){
        this.handleCheckClickRecurse(data,event,index)
      },
      //点击check勾选框,判断是否有children节点 如果有就一并勾选
      handleCheckClickRecurse(data, event, index){
        ...
        if(arr){
          if(data.isChecked){
            ...
            for (let i=0; i<arr.length; i++){
              this.handleCheckClick(arr[i],event,index)
            }
          }else {
            for (let i=0; i<this.checkGroup.length; i++){
              if(this.checkGroup[i] === data){
                this.checkGroup.splice(i,1)
              }
              for (let j=0; j<arr.length; j++){
                if(this.checkGroup[i] === arr[j]){
                  this.handleCheckClick(arr[i],event,index)
                }
              }
            }
      ...
      handleCheckAll() {
        ...
        this.initItems.forEach(item=>{
          this.handleCheckAllRecurse(item,this.checks)
        })
        // this.$emit('on-selection-change', this.checkGroup)
      },
      handleCheckAllRecurse(data,check){
